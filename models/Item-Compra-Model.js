...
const ItemCompra = {
    ...
    async getByCompra(idCompra) { const sql = `SELECT i.*, p.nome, p.url_imagem FROM ItensCompra i INNER JOIN Produto p ON i.idProduto = p.idProduto WHERE i.idCompra = ?`; const [rows] = await db.query(sql, [idCompra]); return rows; },
